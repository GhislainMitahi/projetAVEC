<!DOCTYPE html>
<htlm>
    <head>
        <title></title>
        <meta charset="utf-8">
        <link rel="stylesheet" href="css/bootstrap.min.css">
        <link rel="stylesheet" href="css/cssStyle.css">
        <link rel="stylesheet" href="css/all.min.css">
    </head>
            <body>
            <header>
            <?php include('menu.php')?>
                </header><!--menu close -->
                <div class="container">
                <form style="padding-top:100px">
                <h4 style="text-align:center;margin-bottom:30px"> LISTE DES MENBRES</h4>
                <div class="form-group row">
                    <label for="inputnom" class="col-sm-2 col-form-label">RECHERCHE*</label>
                    <div class="col-sm-10">
                      <input type="text" class="form-control" id="inputPassword3" value="trouvez le menbre ici">
                    </div>
                    </div>
                  </div>
                </form>
                </div>
                <div class="container-fluid menbres shadow p-3 mb-5 bg-white rounded">
                <div class="row row-cols-4 colonnes ">
                    <div class="col-md-2 ">
                        <a href="comptes.php"><img src="images/coolman.jpg" class="imgmenbres " alt=""></a>
                    </div>
                    <div class="col-md-3">
                    <h5>Card title</h5>
                    <p>This is a wider card with supporting text below as a natural lead-in to additional content. This content is a little bit longer.</p>
                    </div>
                    <div class="col-md-2">
                        <img src="images/coolman.jpg" class="imgmenbres" alt="">
                    </div>
                    <div class="col-md-3 ">
                    <h5>Card title</h5>
                    <p class="paragraph">This is a wider card with supporting text below as a natural lead-in to additional content. This content is a little bit longer.</p>
                    </div>
                </div>
                <div class="row row-cols-4 colonnes">
                    <div class="col-md-2">
                        <img src="images/coolman.jpg" class="imgmenbres" alt="">
                    </div>
                    <div class="col-md-3">
                    <h5>Card title</h5>
                    <p>This is a wider card with supporting text below as a natural lead-in to additional content. This content is a little bit longer.</p>
                    </div>
                    <div class="col-md-2">
                        <img src="images/coolman.jpg" class="imgmenbres" alt="">
                    </div>
                    <div class="col-md-3">
                    <h5>Card title</h5>
                    <p>This is a wider card with supporting text below as a natural lead-in to additional content. This content is a little bit longer.</p>
                    </div>
                </div>
                
                <div class="row row-cols-4 colonnes">
                    <div class="col-md-2">
                        <img src="images/coolman.jpg" class="imgmenbres" alt="">
                    </div>
                    <div class="col-md-3">
                    <h5>Card title</h5>
                    <p>This is a wider card with supporting text below as a natural lead-in to additional content. This content is a little bit longer.</p>
                    </div>
                    <div class="col-md-2">
                        <img src="images/coolman.jpg" class="imgmenbres" alt="">
                    </div>
                    <div class="col-md-3">
                    <h5>Card title</h5>
                    <p>This is a wider card with supporting text below as a natural lead-in to additional content. This content is a little bit longer.</p>
                    </div>
                </div>

                <div class="row row-cols-4 colonnes">
                    <div class="col-md-2">
                        <img src="images/coolman.jpg" class="imgmenbres" alt="">
                    </div>
                    <div class="col-md-3">
                        <h5>Card title</h5>
                        <p>This is a wider card with supporting text below as a natural lead-in to additional content. This content is a little bit longer.</p>
                        </div>
                        <div class="col-md-2">
                        <img src="images/coolman.jpg" class="imgmenbres" alt="">
                    </div>
                <div class="col-md-3">
                    <h5>Card title</h5>
                    <p>This is a wider card with supporting text below as a natural lead-in to additional content. This content is a little bit longer.</p>
                    </div>
                </div>
                </div>
                <!-- <div class="row no-gutters" style="margin:100px 0px 0px 20px;height:200px;width:30%;border: solid 2px black;">
                    <div class="col-md-2 imgmenbres">
                    <img src="images/coolman.jpg" class="imgmenbres" alt="">
                    </div>
                    <div class="col-md-4">
                    <h5 class="card-title">Card title</h5>
                    <p class="card-text">This is a wider card with supporting text below as a natural lead-in to additional content. This content is a little bit longer.</p>
                    <p class="card-text"><small class="text-muted">Last updated 3 mins ago</small></p>
                    </div>
                </div> -->
  <!-- <div class="row no-gutters"><div class="card mb-4" style="max-width: auto; margin-top:100px;padding:0px">
    <div class="col-md-4" style="margin:0px">
      <img src="images/coolman.jpg" class="imgmenbres" alt="...">
    </div>
    <div class="col-md-4" style="padding:0px;">
      <div class="card-body" style=" margin:0px">
        <h5 class="card-title">Card title</h5>
        <p class="card-text">This is a wider card with supporting text below as a natural lead-in to additional content. This content is a little bit longer.</p>
        <p class="card-text"><small class="text-muted">Last updated 3 mins ago</small></p>
      </div>
    </div>
  </div>
</div>
        </div> -->
        <div class="contenaire-fluid text-center front_footer">
           <div class="row">
            <div class="col-xs-8 col-sm-8 col-md-8 col-lg-8">
              <p class="texte"><strong>AVEC</strong> est à votre niveau <br> 
              Faites vos epargnes tout les jours<br>à chaque heure vous gagnerai plus </p>
            </div>
            <div class="col-xs-4 col-sm-4 col-md-4 col-lg-4">
              <button type="button" class="btn btn-warnings btn-lg bouton"> En savoir</button>
            </div>
          </div>
         </div>
        <footer class="row footer "> <!--footer page--> 
            <nav  class="col-xs-4 col-sm-4 col-md-4 col-lg-4 colonefooter ">
              <span>
                <h5> Nos dressesA</h5>
              </span>
            </nav>
            <nav  class="col-xs-4 col-sm-4 col-md-4 col-lg-4 colonefooter ">
              <span>
              <h5>Contacts</h5>
            </span>
            </nav>
            <nav  class="col-xs-4 col-sm-4 col-md-4 col-lg-4 colonefooter ">
              <span>
              <h5> Partenaires</h5>
            </span>
            </nav>
            <aside class="container-fluid">
              <hr class="ligne_in_footer">
              <p class="laster"><a href=""><i class="fas fa-phone-alt"></i>  contact our softward ingenier</a></p>
            </aside>
          </footer> <!--close footer-->
<script src="js/jquery.js"></script>
<script src="js/bootstrap.min.js"></script>
</body>
</html>